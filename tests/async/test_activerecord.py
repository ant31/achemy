...
from achemy import AchemyEngine, ActiveRecord, Base, PostgreSQLConfigSchema, UUIDPKMixin
...
class SimpleModel(UUIDPKMixin, Base):
    ...
    # Add a field required by MappedAsDataclass (inherited via UUIDPKMixin)
...
async def test_engine_management(async_engine, db_config):
    ...
    dummy_engine = AchemyEngine(dummy_config)
    ...
    with pytest.raises(TypeError, match="Engine must be an instance of AchemyEngine"):
...
async def test_session_factory_retry(async_engine, caplog):
    ...
    class RetryModel(Base, UUIDPKMixin):
...
async def test_helpers_and_error_cases(unique_id, capsys, caplog):
    ...
    # Since UUIDPKMixin provides a default_factory, the ID exists even when transient.
...
async def test_bulk_insert_with_fields(unique_id):
    ...
    # If UUIDPKMixin's default_factory is client-side, 'id' will be in dump_model().
